import { TEorzeaDateCategory, EorzeaDate } from '../../../src/lib/entities/eorzeaDate';
...
describe('EorzeaDate', () => {
  ...
  `('constructor "$input" -> "$epoch"', ({ input, epoch, year, month, day, hour, minute, second, millisecond }) => {
    const date = new EorzeaDate(new Date(input))
  ...
  `('string "$dateStr" "$timeStr"', ({ year, month, day, hour, minute, second, dateStr, timeStr }) => {
    const epoch = createEpoch(year, month, day, hour, minute, second)
    const date = new EorzeaDate(new Date(epoch))
    expect(date.year).toBe(year);
    expect(date.month).toBe(month);
    expect(date.day).toBe(day);
    expect(date.hour).toBe(hour);
    ...
    expect(date.toString()).toMatch(/^\d+\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}.\d{3}$/);
    ...
    expect(date.toJSON().et).toMatch(/^\d+\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}.\d{3}$/)
  });
  ...
  `('add "$category" "$value"', ({ category, value, year, month, day, hour, minute, second, millisecond }) => {
    const date = new EorzeaDate(new Date(0))
